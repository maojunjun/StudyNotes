...
var kval=null; var  urlval=null; var  modeval=null; var  cacheval=null; var menuvval=null;
function showWindow(k, url, mode, cache, menuv) {
    if (k == 'newthread'|| k=='reply')
       checkUserStatus(k, url, mode, cache, menuv, checkStatus);
    else
       sshowWindow(k, url, mode, cache, menuv);
}

var xmlHttpReg = null;
function checkUserStatus(k, url, mode, cache, menuv,callback) {
       if (discuz_uid != 0 ) {
	  var fid = getUrlParam(url, "fid");
	  var sendUrl = "forum.php?operteType=checkUserStatus&fid="+fid;
          selfAjax(sendUrl, k, url, mode, cache, menuv,callback);
       } else {
          sshowWindow(k, url, mode, cache, menuv);
       }
}

function selfAjax(url, k, surl, mode, cache, menuv,callback) {
        ...
        if (xmlHttpReg != null) {
              ...
              xmlHttpReg.onreadystatechange = function() {
                 callback(k, surl, mode, cache, menuv);
              };
        }
}

function checkStatus(k, url, mode, cache, menuv) {
    if (xmlHttpReg.readyState == 4) {
        if (xmlHttpReg.status == 200) {
             var result = eval("(" + xmlHttpReg.responseText + ")");
             if(result.astatus == 0){
               if(result.devStatus==0){
                  showDialog("您的百度开发者状态为：未激活，请先激活!!!",'confirm','提示信息',function(){
                     window.open("http://developer.baidu.com/user/reg");
                  },0,null,null,'激活');
               }else if(result.devStatus==1){
                  showDialog("您的百度开发者状态为：未提交身份认证，请提交身份认证!!!",'confirm','提示信息',function(){
                         window.open("http://developer.baidu.com/user/info");
                  },0,null,null,'提交');
               }else if(result.devStatus==2){
                  showDialog("您的百度开发者状态为：待审核，暂不能发帖!!!",'confirm','提示信息',function(){
                         window.open("http://developer.baidu.com/user/info");
                  },0,null,null,'查看');
               }else if(result.devStatus==3){
                  showDialog("您的百度开发者状态为：审核不通过，暂不能发帖!!!",'confirm','提示信息',function(){
                         window.open("http://developer.baidu.com/user/info");
                  },0,null,null,'查看');
               }else if(result.devStatus==5){
                  showDialog("您的百度开发者状态为：被拉黑，暂不能发帖!!!",'confirm','提示信息',function(){
                         window.open("http://developer.baidu.com/user/info");
                  },0,null,null,'查看');
               }else if(result.devStatus==4){
                  if (result.lbsDevStatus == 1) {
                     showDialog("您还未激活LBS开发者，请激活!!!",'confirm','提示信息',function(){
                          hideMenu('fwin_dialog', 'dialog');
                          kval = k; urlval = url; modeval = mode; cahceval = cache; menuvval = menuv;
                          sshowWindow('newthread', 'forum.php?operteType=activateAgreement');
                     },0,null,null,'激活');
                  } else if(result.lbsDevStatus == 0){
                     sshowWindow(k, url, mode, cache, menuv);
                  }
               }
             }else{
...
function agreeLbsDevAgreement(k) {
     hideWindow(k);
     selfAjax("forum.php?operteType=activateLbsDev", kval, urlval, modeval, cacheval, menuvval,activateStatus);
}

function activateLBSDev(k, url, mode, cache, menuv){
      hideMenu('fwin_dialog', 'dialog');
      selfAjax("forum.php?operteType=activateLbsDev", k, url, mode, cache, menuv,activateStatus);
}

var activateNum = 0;
function activateStatus(k, url, mode, cache, menuv) {
    if (xmlHttpReg.readyState == 4) {
         if (xmlHttpReg.status == 200) {
           var result = eval("(" + xmlHttpReg.responseText + ")");
           if (result.activateStatus == 0) { 
                   sshowWindow(k, url, mode, cache, menuv);
           }else{
               activateNum++;
               showDialog("抱歉，激活失败!!!",'confirm','提示信息',activateLBSDev,0,null,null,'再来一次');
               if(activateNum<=5)
                  function activateLBSDev(){
                         hideMenu('fwin_dialog', 'dialog');
                         selfAjax("forum.php?operteType=activateLbsDev", k, url, mode, cache, menuv,activateStatus);
                  }
               else
                  showDialog("操作太频繁，请稍候再试，谢谢！！！",'alert','提示信息');
           }
         }
    }
}
...
function sshowWindow(k, url, mode, cache, menuv) {
